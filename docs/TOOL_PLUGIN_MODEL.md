...
### 1.1 默认开发形态（Skill-first，非专业）

**后续工具开发默认不是「直接写 Tool 包」**，而是：
...
- **非专业作者**：只维护 **Platform Skill v1**（见 [`SKILL_PLATFORM.md`](./SKILL_PLATFORM.md)）；UI 默认 **schema 表单**，不写 React。  
- **平台封装 Skill**：`.cursor/skills/iisp-skill-author` + `iisp-skill-pack` 将 Skill 转为可加载 Tool，**强制服从** Contract、Catalog、Shell UI。  
- **专业兜底**：`iisp-tool-package` / `iisp-create-tool` 仅在 script 不够、需 custom UI 时使用。
...
| 组合 | workflow_engine（迁移中） | Catalog Pipeline + Kestra/cron |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.0 | 2026-06-09 | 工具包标准、四形态、lib 分层、终态 Checklist |
