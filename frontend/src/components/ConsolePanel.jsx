...
import ConsoleVizBlock from './ConsoleVizBlock';

export function ConsoleOutputContent({ content }) {
  const [viewIndex, setViewIndex] = useState(null);
  const { viewMatches, vizMatches, cleanOutput } = useMemo(
  ...
  return (
    <>
      ...
      {vizMatches.length > 0 && (
        <div className="console-viz-list">
          {vizMatches.map((spec, index) => (
            <ConsoleVizBlock key={`viz-${index}-${spec.title || ''}`} spec={spec} index={index} />
          ))}
        </div>
      )}
  ...
  return (
    <div className="console-panel" style={{ display: 'block' }}>
      ...
      <div className={`console-body ${type || ''}`}>
        <ConsoleOutputContent content={content} />
      </div>
    </div>
  );
}
...
export const PYTHON_HELP = [
  ['view(df, description=None)', '弹窗查看 DataFrame 表格'],
  ['info(df)', '打印 DataFrame 基本信息到控制台'],
  ['describe_df(df, description=None)', '弹窗查看统计描述'],
  ['plot_box_counts(df, title=...)', '柱状图：ext 检测框各类别数量（显示在执行输出区）'],
  ['plot_value_counts(df, column, title=...)', '柱状图：指定列取值频次'],
  ['plot_numeric_hist(df, column, bins=20)', '直方图：数值列分布'],
  ['plot_bars(df, x=..., y=...)', '通用柱状图（如 count_category_boxes 结果）'],
