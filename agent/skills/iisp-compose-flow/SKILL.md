...
- **首选（带中文注释）**：[`iisp-catalog/pipelines/kestra/closed_loop_demo_smoke.yaml`](../../../iisp-catalog/pipelines/kestra/closed_loop_demo_smoke.yaml)  
- Kestra：[`docs/examples/kestra/daily_ng_curation.yaml`](../../../docs/examples/kestra/daily_ng_curation.yaml)  
- 注释规范：[`docs/FLOW_YAML_COMMENTS.md`](../../../docs/FLOW_YAML_COMMENTS.md)  
...
生成的每个 Flow **必须**带中文注释，便于非 Kestra 专家阅读。规范见 [`docs/FLOW_YAML_COMMENTS.md`](../../../docs/FLOW_YAML_COMMENTS.md)。

**文件头**（2 行）：

- 流程一句话（步骤链）
- 关键 ID 接力（如 `task_id → batch_id → …`）

**每个 HTTP 工具步骤前**（4 行）：

1. `【步骤 {id}】工具 {tool_id} — {中文干什么}`  
2. `入参：字段 ← 来源`（写清来自 `inputs.xxx` 或 `某步.outputs.yyy`）  
3. `出参：字段 → 留给哪些下游步骤`  
4. `上游包：inputs.upstream = …`（无上游则写「无，第一步」）

**Pause / If** 步骤前写 1～2 行说明卡点条件或分支含义。

**禁止**：只交裸 `fromJson(outputs…)` 而不写对应人话注释。
